...
import { Card, Form, Grid, Hint, Label, Meta, Page, QrImage, Title } from './styled'
...
function QrBody({ restaurant }: { restaurant: Restaurant }) {
  ...
  return (
    <Page>
      ...
      {page.query.isLoading && <Skeleton height="220px" />}

      {page.query.isSuccess && (page.query.data?.length ?? 0) === 0 && (
        <EmptyState emoji="📱" title={t('qr.empty')} hint={t('qr.emptyHint')} />
      )}

      <Grid>
        {(page.query.data ?? []).map((qr) => (
          <Card key={qr.id}>
