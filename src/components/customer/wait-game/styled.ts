...
export const Arena = styled.div`
  ...
  background:
    ...
    linear-gradient(180deg, ${palette.cream} 0%, ${palette.mangoWash} 100%);
...
export const DishButton = styled.button<{
  ...
  border: 2px solid ${palette.ink};
  border-radius: ${radii.full};
  background: ${({ $rare }) => ($rare ? palette.mango : palette.white)};
...
export const PopLabel = styled.span<{ $x: number; $y: number }>`
  position: absolute;
  left: ${({ $x }) => `${$x}%`};
  top: ${({ $y }) => `${$y}%`};
  z-index: 3;
  ...
  animation: ${popScore} 520ms ease-out both;
`
